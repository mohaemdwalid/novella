...
function buildProductUrlWithPreservedParams(url, params = {}) {
  const urlObject = new URL(url, window.location.origin);
  const currentSearchParams = new URLSearchParams(window.location.search);
  const mergedSearchParams = new URLSearchParams(urlObject.search);

  ["preview_theme_id", "_fd", "pb"].forEach(paramName => {
    if (currentSearchParams.has(paramName) && !mergedSearchParams.has(paramName)) {
      mergedSearchParams.set(paramName, currentSearchParams.get(paramName));
    }
  });

  Object.entries(params).forEach(([paramName, paramValue]) => {
    if (paramValue === null || paramValue === undefined || paramValue === "") {
      mergedSearchParams.delete(paramName);
      return;
    }

    mergedSearchParams.set(paramName, paramValue);
  });

  const search = mergedSearchParams.toString();
  return `${urlObject.pathname}${search ? `?${search}` : ""}`;
}
...
let heroBanner = document.querySelector(".sage-hero-banner");
...
// add loading lazy attribute to div.sage-hero__inner > div.media > video > img
document
  .querySelectorAll(".sage-hero__inner > .media > video > img")
  .forEach(function (el) {
...
function setRootCustomProperties() {
  ...
  if (heroBanner) {
    setCustomProperty("--sage-hero-banner-top", `${parseFloat(getCoordinates(heroBanner).top)}px`);
    setCustomProperty("--sage-hero-banner-bottom", `${parseFloat(getCoordinates(heroBanner).bottom)}px`);
  }
  ...
  // get data-header-menu-text-color of first .sage-hero__inner under .sage-hero-banner element
  if (heroBanner) {
    let heroInner = heroBanner.querySelectorAll(".sage-hero__inner");
...
const releaseBodyScrollWhenIdle = () => {
  if (!document.body.classList.contains('template--product')) return;

  const hasBlockingOverlay =
    document.querySelector('quick-cart-drawer.is--open') ||
    document.querySelector('shop-the-look-drawer.is--open') ||
    document.querySelector('modal-dialog[open]') ||
    document.querySelector('product-modal[open]') ||
    document.querySelector('.pswp.pswp--open') ||
    document.querySelector('details.menu-opening') ||
    document.body.classList.contains('age-verification-popup-is-open') ||
    document.body.classList.contains('tooltip-mobile-open');
...
window.releaseBodyScrollWhenIdle = releaseBodyScrollWhenIdle;

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', releaseBodyScrollWhenIdle);
} else {
  releaseBodyScrollWhenIdle();
}

window.addEventListener('pageshow', releaseBodyScrollWhenIdle);
document.body.addEventListener('modalClosed', () => {
  requestAnimationFrame(releaseBodyScrollWhenIdle);
});
...
let hero =
  document.querySelector(".sage-hero-banner") ||
      ...
      this.querySelectorAll("[data-product-link]").forEach(
        linkItem => {
          if (currentVariantId) {
            ...
            const newLink = buildProductUrlWithPreservedParams(currentLink, {
