 ...
 class ProductMedia extends HTMLElement {
    ...
    photoSwipeLightboxInit() {
      ...
      photoSwipeLightboxInstance.on("closingAnimationStart", () => {
        ...
        window.releaseBodyScrollWhenIdle?.();
      });

      photoSwipeLightboxInstance.on("close", () => {
        document.body.classList.remove("overflow-hidden");
        window.releaseBodyScrollWhenIdle?.();
      });
    ...
    setActiveModalOpener(swiper) {
      const currentSlide = swiper.slides.find(s => s.classList.contains('swiper-slide-active'));
      if (!currentSlide) return;
      
      const mediaId = currentSlide.dataset.mediaId;
      const activeOpener = this.querySelector(`${this.selectors.modalOpeners}.is-active`);
      if (activeOpener) activeOpener.classList.remove('is-active');
      
      const newOpener = this.querySelector(`${this.selectors.modalOpeners} [data-media-id="${mediaId}"]`);
      if (newOpener) newOpener.parentElement.classList.add('is-active');
      
      if (currentSlide.dataset.mediaType === 'model') {
        const xrButton = this.querySelector(this.selectors.btnXr);
        if (xrButton) xrButton.dataset.shopifyModel3dId = mediaId;
      }
    }
