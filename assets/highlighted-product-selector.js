  ...
  class ProductSelector extends HTMLElement {
    ...
    updateOptions() {
      this.querySelectorAll('input[type="radio"]').forEach(input => {
        ...
        if (nextEl) {
          nextEl.classList.toggle("is-active", !input.disabled && input.checked);
        }
      });
      
      const formInputs = this.querySelectorAll('input[type="radio"]:checked, select');
      this.options = Array.from(formInputs, el => ({
        name: el.dataset.name || el.name,
        value: el.value
      }));
    }
