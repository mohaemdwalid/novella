...
class ProductMediaInfo extends HTMLElement {
  ...
  init() {
    let sliderOffsetWidth = document.querySelector('.main-product__media--slider')?.offsetWidth || 330;
    const gridEl = document.querySelector('.main-product__media--grid-item');
    if (gridEl && !DeviceDetector.isMobile()) {
      sliderOffsetWidth = gridEl.offsetWidth;
    }
    const maxInfoWidth = (sliderOffsetWidth - 48) / 2;

    const textNode = this.querySelector('p:last-child');
    if (!textNode) return;

    const textWidth = textNode.offsetWidth;
    const hiddenNode = this.querySelector('p[aria-hidden]');

    if (textWidth > maxInfoWidth) {
      this.classList.remove('animation-stopped');
      this.style.setProperty('--marquee-speed', `${(textWidth / maxInfoWidth) * 8}s`);
