class ProductModal extends ModalDialog {
  ...
  showActiveMedia(activeMediaType = 'videos') {
    if (activeMediaType !== 'videos') {
      const mediaId = this.openedBy.getAttribute('data-media-id');
      const targetMedia = this.querySelector(`[data-media-id="${mediaId}"]`);
      
      targetMedia?.loadContent();
      
      this.querySelectorAll('[data-media-id]').forEach(media => {
        const isTarget = media === targetMedia;
        media.parentElement.classList.toggle('swiper-slide', isTarget);
        media.parentElement.classList.toggle('hidden', !isTarget);
      });
