class PurchaseOptions extends HTMLElement {
  ...
  isOneTimePurchase() {
    return this.querySelector('[name="selling_plan_group"]')?.value === "one-time";
  }
  ...
  updateProductSelector(sellingPlanId) {
    if (this.productBase) {
      return this.updateProductBase(sellingPlanId);
    }
    ...
    const isAvail = this.productSelector.currentVariant.available;
    if (isAvail && sellingPlanId) {
