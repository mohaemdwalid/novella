  ...
  customElements.define(
    ...
    class RecipientForm extends HTMLElement {
      constructor() {
        ...
        if(this.sendonInput) {
          this.sendonInput.addEventListener("input", function () {
      ...
      clearErrorMessage() {
        ...
        this.recipientInputFields().forEach(inputElement => {
...
document.addEventListener('DOMContentLoaded', () => {
  const recipientForm = document.querySelector('.gift-card-recipient-form');

  recipientForm.addEventListener('keydown', (event) => {
    if (event.key === 'Enter' || event.keyCode === 13) {
      const recipientCheckboxLabel = recipientForm.querySelector('.recipient-checkbox');
      if (recipientCheckboxLabel) {
        recipientCheckboxLabel.click();
      }
      const recipientCheckbox = recipientForm.querySelector('input[type="checkbox"]');
