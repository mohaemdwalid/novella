...
async function initMap() {
  ...
  const extractNodes = (selector, type = "textContent") => 
    Array.from(document.querySelectorAll(selector)).map(el => el[type]);
  ...
  const [
    ...
    extractNodes(".store-locator-card__longitude"),
    extractNodes(".store-locator-card__latitude"),
    extractNodes(".store-locator-card__address"),
    extractNodes(".store-locator-card__opening-hour", "innerHTML"),
    extractNodes(".store-locator-card__actions", "innerHTML"),
    extractNodes(".store-locator-card__content--country", "innerHTML"),
    extractNodes(".store-locator-card__content--city", "innerHTML"),
    extractNodes(".store-locator-card__address", "innerHTML"),
    extractNodes(".store-locator-card__phone", "innerHTML"),
    extractNodes(".store-locator-card__mail", "innerHTML")
  ];

  const coordinateTitle = extractNodes(".store-locator-card__coordinate-title")
  ...
  for (let i = 0; i < locations.length; i++) {
    ...
    closeBtn.addEventListener("click", e => {
      ...
      document.querySelectorAll(".store-locator__map--custom-marker--point").forEach(point => point.classList.remove("hidden"));
      showPin();
    });
    ...
    customMarker.append(customMarkerLabel, customMarkerPoint);
    ...
    google.maps.event.addListener(
      ...
      (function (marker, count) {
        return function () {
          ...
          const allMarkers = document.querySelectorAll(".store-locator__map--custom-marker");
          allMarkers.forEach(item => item.classList.remove("store-locator__map--custom-marker--active"));
          ...
          document.querySelectorAll(".store-locator__map--custom-marker--label").forEach(label => {
            label.style.display = label.parentElement === clickedMarker ? "flex" : "none";
          });
    ...
    function showPin () {
      const isPinActive = document.querySelector('.store-locator').getAttribute('data-show-pin') === 'true';
      document
        .querySelectorAll(".store-locator__map--custom-marker--label")
        .forEach(label => (label.style.display = isPinActive ? "flex" : "none"));
    }
